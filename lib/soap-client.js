...
class SOAPClient extends FetchWrap {
		...
		function recursivelyParseDom(json, element) {
			...
			const childText =
				...
				Array.from(element.childNodes).find(
					(child) =>
						child.nodeType === child.TEXT_NODE && child.textContent
				);
	...
	patchOrPostOpts(method, url, body, getParamsObj) {
		...
		return super.executeRequest(url, {
			method,
			headers, // serialize doc works in chrome, what about node?
			body: serializer.serializeToString(xmlDoc),
		});
	}
}
...
class JsonToXML {
	...
	convert() {
		...
		const result = this.keys.reduce(
			...
			this.xmlDoc.lastChild // browser must be children[0]
		);
		function recursivelyParseJson(xmlNode, key) {
			if (inst.isTag(key)) {
				...
				if (inst.currentDepth instanceof Array && inst.arrIndex === 1) {
					inst.arrayWrappers.push(xmlNode);
				}
		...
		function recursivelyParseJson(xmlNode, key) {
			if (inst.isTag(key)) {
				...
				if (typeof xmlNode.json[key] === 'string') {
					...
					if (xmlNode.json[key] instanceof Array) {
						if (xmlNode.currentIndex >= currentDepth.length) {
							xmlNode.currentIndex = 0;
						}
						currentDepth = xmlNode.json[key][xmlNode.currentIndex];
